...
/*
 * GRA v414 — estabilidade das transições entre avaliações somativas.
 *
...
(function(){
  ...
  const filterStats=window.__GRA_V414_FILTER_STATS__={hits:0,misses:0,resets:0};
  ...
  function wrapFilteredRows(){
    ...
    if(typeof current!=='function'||current.__v414TransitionTickCache||current.__graV409HardSchoolFilter)return;
    const base=current;
    ...
    wrapped.__v414TransitionTickCache=true;
  ...
  function wrapRender(){
    ...
    if(typeof current!=='function'||current.__v414Transition)return;
    const base=current;
  ...
  async function settleTransition(token,ensureSimulado){
    try{
      ...
      failedTransitions++;
      console.error('v414: falha ao preparar o recorte do Simulado 2026',error);
    }
    ...
    try{
      ...
      failedTransitions++;
      console.error('v414: falha na renderização consolidada',error);
    }finally{
      pending=false;
      flushing=false;
      document.documentElement.removeAttribute('data-gra-simulado-transition');
  ...
  function install(){
    wrapFilteredRows();
    wrapRender();
    document.documentElement.dataset.graV414TransitionStability='installed';
  }
  ...
  window.__GRA_V414_TRANSITION_STABILITY__={
    ...
    audit(){return {
      version:VERSION,
      installed:document.documentElement.dataset.graV414TransitionStability==='installed',
      active,
      pending,
      suppressedRenders,
      passedRenders,
